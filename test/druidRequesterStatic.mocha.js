...
describe('Druid requester static data source', function () {
    ...
    it('works with regular scan (list)', () => {
      const requester = druidRequester({
        query: {
          ...
          resultFormat: 'list',
          limit: 4,
          columns: ['page', 'channel', 'comment', 'added'],
        },
      });

      return toArray(requester).then(res => {
        expect(res.length).to.equal(4);
        expect(res).to.deep.equal([
          {
            added: 0,
        ...
        expect(res).deep.to.equal([
          {
            '!H_P95': {
              breaks: [
                -19985.333984375, -499, 18987.333984375, 38473.66796875, 57960, 77446.3359375,
                96932.671875, 116419,
              ],
              counts: [0, 2677, 2, 1, 0, 0, 1],
            },
            'Count': 2662,
            'P95': 92.884995,
            'timestamp': new Date('2015-09-12T00:00:00.000Z'),
          ...
          {
            '!H_P95': {
              breaks: [-28350.5, -498, 27354.5, 55207, 83059.5, 110912, 138764.5, 166617],
              counts: [0, 11429, 8, 2, 0, 1, 2],
            },
            'Count': 11392,
            'P95': 130.39272,
            'timestamp': new Date('2015-09-12T01:00:00.000Z'),
    ...
    it('works with DruidSQL empty result', () => {
      return toArray(
        druidRequester({
          query: {
            query:
              "SELECT page, COUNT(*) AS Cnt FROM wikipedia WHERE cityName = 'lol_lol_lol_lol' GROUP BY page LIMIT 5",
          },
        }),
      ).then(res => {
        expect(res).to.deep.equal([]);
  ...
  describe('timeout', () => {
    it('works in simple case', () => {
      const timeoutDruidRequester = druidRequesterFactory({
        host: info.druidHost,
        timeout: 50,
      });
  ...
  describe('cancel', () => {
    it('works in precancel case', () => {
      ...
      const cancelDruidRequester = druidRequesterFactory({
        host: info.druidHost,
        timeout: 5000,
        cancelToken: token,
      });
